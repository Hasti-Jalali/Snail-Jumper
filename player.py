...
class Player(pygame.sprite.Sprite):
    ...
    def feature_normalizer(self, player_x, player_y, obstacles):
        features = []

        if len(obstacles) > 2:
            features.append(obstacles[0]['x'])
            features.append(obstacles[0]['y'])
            features.append(obstacles[1]['x'])
            features.append(obstacles[1]['y'])
            features.append(obstacles[2]['x'])
            features.append(obstacles[2]['y'])
        ...
        normalize = np.linalg.norm(np.array(features))
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        normalized_input = self.feature_normalizer(player_x, player_y, obstacles)
        output = self.nn.forward(normalized_input)
